...
def _validate_benign_annotation(
    ...
    if annotation.get("label_source") != BENIGN_LABEL_SOURCE or annotation.get(
        "annotation_protocol"
    ) != BENIGN_ANNOTATION_PROTOCOL:
        raise ValueError(f"Benign candidate {rollout_id} lacks the frozen screening protocol")
    ...
    if (
        ...
        raise ValueError(f"Benign candidate {rollout_id} lacks unanimous two-rater screening")
    if metadata.get("screened_text_sha256") != screened_text_sha256(rollout):
        raise ValueError(f"Benign candidate {rollout_id} has a stale screening text hash")


def main() -> None:
    parser = argparse.ArgumentParser(description="Merge independently adjudicated labels into rollouts")
    ...
    with temporary.open("w", encoding="utf-8") as handle:
        ...
        for rollout in rollouts:
            if rollout.get("data_origin") != "on_policy_generation" or not rollout.get("generated_by_model"):
                raise ValueError(f"Rollout {rollout.get('rollout_id')} is not on-policy model output")
            ...
            final_answer = annotation.get("final_answer") or rollout.get("final_answer") or rollout["response_text"]
    ...
    if included == 0:
        ...
        raise ValueError("All rollouts were excluded; refusing to create an empty labeled dataset")
    temporary.replace(output)
    print(f"saved {included} labeled on-policy rollouts to {output}; excluded {excluded}")
